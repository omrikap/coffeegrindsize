...
reference_objects_dict = {"Custom": None, "Canadian Quarter": 23.81, "Canadian Dollar": 26.5, "Canadian Dime": 18.03,
                          "Canadian Two Dollars": 28.0, "Canadian Five Cents": 21.3, "US Quarter": 24.26,
                          "US Dollar": 26.92, "US Dime": 17.91, "US Penny": 19.05, "2 Euros": 25.75, "1 Euro": 23.25,
                          "50 Euro Cents": 24.25, "20 Euro Cents": 22.25}
...
class coffeegrindsize_GUI:
    ...
    def __init__(self, root):
        ...
        self.pixel_length_var, self.pixel_length_id, self.simple_pixel_length_id = self.label_entry(def_pix_len,
                                                                                                    "Reference Pixel Length:",
                                                                                                    "pix",
                                                                                                    entry_id=True,
                                                                                                    clear_on_click=True)
        self.physical_length_var, self.physical_length_id, self.simple_physical_length_id = self.label_entry(
            reference_objects_dict["Custom"], "Reference Physical Size:", "mm", entry_id=True,
            event_on_entry="update_pixel_scale", clear_on_click=True)
        ...
        self.reference_object = self.dropdown_entry("Reference Object:", list(reference_objects_dict.keys()),
        ...
        self.max_cluster_axis_var = self.label_entry(def_max_cluster_axis, "Maximum Cluster Diameter:", "pix",
                                                     advanced=True, clear_on_click=True)
        ...
        self.hist_choices = ["Number vs Diameter", "Number vs Surface", "Number vs Volume", "Mass vs Diameter",
                             "Mass vs Surface", "Mass vs Volume", "Available mass vs Diameter",
                             "Available mass vs Surface", "Available mass vs Volume", "Surface vs Diameter",
                             "Surface vs Surface", "Surface vs Volume"]
        self.hist_codes = ["num_diam", "num_surf", "num_vol", "mass_diam", "mass_surf", "mass_vol", "av_mass_diam",
                           "av_mass_surf", "av_mass_vol", "surf_diam", "surf_surf", "surf_vol"]
        ...
        self.legend_choices = ["Best", "Upper Right", "Upper Left", "Lower Right", "Lower Left", "Center Right",
                               "Center Left", "Lower Center", "Upper Center", "Right", "Center"]
        self.legend_type = self.dropdown_entry("Label Position:", self.legend_choices, self.change_histogram_type)

        # Label for the data
        self.data_label_var = self.label_entry("Current Data", "Data Label:", "", columnspan=2,
                                               width=self.width_entries * 3, event_on_enter="create_histogram")

        # Label for the comparison data
        self.comparison_data_label_var, self.comparison_data_label_id, self.simple_comparison_data_label_id = self.label_entry(
            "Comparison Data", "Comparison Label:", "", entry_id=True, columnspan=2, width=self.width_entries * 3,
        ...
        self.xmin_var, self.xmin_var_id = self.label_entry(def_min_x_axis, "Min. X Axis:", "", entry_id=True, addcol=1,
                                                           event_on_enter="create_histogram", advanced=True,
                                                           clear_on_click=True)
        ...
        self.xlog_var.set(1)
        xlog_checkbox = Checkbutton(self.frame_options, text="Log X axis | ", variable=self.xlog_var,
                                    command=self.xlog_event)
        xlog_checkbox.grid(row=self.options_row, columnspan=1, sticky=E, column=0)

        self.xmax_var, self.xmax_var_id = self.label_entry(def_max_x_axis, "Max. X Axis:", "", entry_id=True, addcol=1,
                                                           event_on_enter="create_histogram", advanced=True,
                                                           clear_on_click=True)
        ...
        nbins_auto_checkbox = Checkbutton(self.frame_options, text="Automated bins | ", variable=self.nbins_auto_var,
        ...
        # X axis range for the histogram figure
        self.nbins_var, self.nbins_var_id = self.label_entry(10, "Num. bins:", "", entry_id=True, addcol=1,
                                                             event_on_enter="create_histogram", advanced=True,
                                                             clear_on_click=True)
        ...
        self.output_dir_var, self.output_dir_label_id = self.label_entry(self.output_dir, "", "", entry_id=True,
                                                                         columnspan=2, width=self.width_entries * 3,
                                                                         advanced=True)
        ...
        self.session_name_var = self.label_entry(def_session_name, "Base of File Names:", "", columnspan=2,
                                                 width=self.width_entries * 3, advanced=True)
        ...
        choices = [original_image_display_name, threshold_image_display_name, outlines_image_display_name,
        ...
        simple_reset_params_button = Button(self.simple_frame_options, text="Reset All Parameters",
        ...
        title_label = Label(self.frame_stats, text="Properties of the Particle Distribution:", font='Helvetica 16 bold',
                            bg=frame_stats_bg)
        ...
        diam_average_label = Label(self.frame_stats, text="Average Diameter:", bg=frame_stats_bg,
                                   font='Helvetica 14 bold')
        diam_average_label.grid(row=stats_row, sticky=E, column=stats_column)
        diam_average_entry = Label(self.frame_stats, textvariable=self.diam_average_var, width=stats_entry_width,
                                   bg=frame_stats_bg)
        ...
        diam_stddev_label = Label(self.frame_stats, text="Scatter in Diameter:", bg=frame_stats_bg,
                                  font='Helvetica 14 bold')
        diam_stddev_label.grid(row=stats_row, sticky=E, column=stats_column)
        diam_stddev_entry = Label(self.frame_stats, textvariable=self.diam_stddev_var, width=stats_entry_width,
                                  bg=frame_stats_bg)
        ...
        surf_average_label = Label(self.frame_stats, text="Average Surface:", bg=frame_stats_bg,
                                   font='Helvetica 14 bold')
        surf_average_label.grid(row=stats_row, sticky=E, column=stats_column)
        surf_average_entry = Label(self.frame_stats, textvariable=self.surf_average_var, width=stats_entry_width,
                                   bg=frame_stats_bg)
        ...
        surf_stddev_label = Label(self.frame_stats, text="Scatter in Surface:", bg=frame_stats_bg,
                                  font='Helvetica 14 bold')
        surf_stddev_label.grid(row=stats_row, sticky=E, column=stats_column)
        surf_stddev_entry = Label(self.frame_stats, textvariable=self.surf_stddev_var, width=stats_entry_width,
                                  bg=frame_stats_bg)
        ...
        self.image_canvas = Canvas(self.container_options, width=self.canvas_width, height=self.canvas_height,
                                   bg=image_canvas_bg)
        ...
        self.noimage_label = Label(self.image_canvas, text="No Image Loaded", anchor=CENTER, bg=image_canvas_bg,
                                   font='Helvetica 22 bold', width=self.canvas_width, height=self.canvas_height)
        ...
        open_image_button = Button(toolbar, text="Open Image", command=lambda: self.open_image(None),
        ...
        ref_obj_button = Button(toolbar, text="Select Reference Object",
        ...
        region_button = Button(toolbar, text="Select Analysis Region", command=lambda: self.select_region(None),
                               highlightbackground=toolbar_bg)
        region_button.pack(side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady)

        # Button to apply image threshold
        threshold_image_button = Button(toolbar, text="Threshold Image", command=lambda: self.threshold_image(None),
                                        highlightbackground=toolbar_bg)
        ...
        psd_button = Button(toolbar, text="Launch Particle Detection", command=lambda: self.launch_psd(None),
                            highlightbackground=toolbar_bg)
        psd_button.pack(side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady)

        # Button to erase some clusters
        erase_button = Button(toolbar, text="Erase Clusters", command=lambda: self.erase_clusters(None),
                              highlightbackground=toolbar_bg)
        erase_button.pack(side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady)

        # Button to display histogram figures
        histogram_button = Button(toolbar, text="Create Histogram", command=lambda: self.create_histogram(None),
                                  highlightbackground=toolbar_bg)
        histogram_button.pack(side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady)

        # Read Blog Button
        # Button to open blog
        blog_button = Button(toolbar, text="Read Coffee Ad Astra Blog", command=self.blog_goto,
                             highlightbackground=toolbar_bg)
        blog_button.pack(side=RIGHT, padx=self.toolbar_padx, pady=self.toolbar_pady)

        # Downsample button
        downsample_button = Button(toolbar2, text="Reduce Image Quality", command=self.downsample_image,
                                   highlightbackground=toolbar_bg)
        downsample_button.pack(side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady)

        # Button to load data from disk
        load_data_button = Button(toolbar2, text="Load Data", command=lambda: self.load_data(None),
                                  highlightbackground=toolbar_bg)
        load_data_button.pack(side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady)

        # Button to load comparison data from disk
        load_comparison_data_button = Button(toolbar2, text="Load Comparison Data",
        ...
        flush_comparison_data_button = Button(toolbar2, text="Flush Comparison Data",
                                              command=self.flush_comparison_data, highlightbackground=toolbar_bg)
        ...
        save_button = Button(toolbar2, text="Save Data", command=lambda: self.save_data(None),
                             highlightbackground=toolbar_bg)
        save_button.pack(side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady)

        # Button to save histogram to disk
        savehist_button = Button(toolbar2, text="Save View", command=lambda: self.save_histogram(None),
                                 highlightbackground=toolbar_bg)
    ...
    def label_entry(self, default_var, text, units_text, columnspan=None, width=None, entry_id=False,
                    clear_on_click=False, event_on_entry=None, addcol=0, event_on_enter=None, advanced=False):
        ...
        if self.polygon_alpha is not None:
            ...
            if npoly > 1:
                ...
                for i in range(npoly - 1):
                    ...
                    # Redraw line
                    line_obj = self.image_canvas.create_line(line_x_start, line_y_start, line_x_end, line_y_end,
                                                             fill="green")
    ...
    def release_mouse(self, event):
        ...
        if self.mouse_click_mode == "SELECT_REFERENCE_OBJECT":
            ...
            self.status_var.set(
                "The length of the the reference object was set to " + self.pixel_length_var.get() + " pixels... Now select its physical size with the library in the Reference Object dropdown menu, or with a custom Reference Physical Size...")
    ...
    def motion(self, event, only_refresh_display=False):
        ...
        if self.mouse_click_mode == "SELECT_REGION":
            ...
            if self.polygon_alpha is not None:
                ...
                # Redraw line
                self.selreg_current_line = self.image_canvas.create_line(line_x_start, line_y_start, line_x_end,
                                                                         line_y_end, fill="green")
    ...
    def downsample_image(self):
        ...
        # Resize image
        self.img_source = self.img_source.resize((int(float(self.img.size[0]) / 2), int(float(self.img.size[1]) / 2)),
                                                 Image.ANTIALIAS)
        self.img = self.img_source
        ...
        # Update the user interface status
        self.status_var.set(
            "The Image was Downsampled by a Factor two... Current Size is (" + str(self.img.size[0]) + ", " + str(
                self.img.size[1]) + ")")
    ...
    def open_image(self, event):
        ...
        image_filename = filedialog.askopenfilename(initialdir=self.output_dir, title="Select a PNG image", filetypes=(
        ("png files", "*.png"), ("jpeg files", "*.jpg"), ("jpeg files", "*.jpeg"), ("all files", "*.*")))
    ...
    def threshold_image(self, event):
        ...
        if self.polygon_alpha is not None:
            ...
            self.mask_threshold = (
            self.mask_threshold[0][np.where(contained)[0]], self.mask_threshold[1][np.where(contained)[0]])

            # Keep a list of edge pixels
            self.mask_threshold_edge = self.points_along_polygon(self.mask_threshold[1].astype(float),
                                                                 self.mask_threshold[0].astype(float),
                                                                 self.polygon_alpha, self.polygon_beta)
    ...
    def launch_psd(self, event):
        ...
        for i in range(nmask):
            ...
            if i % 10 == 0:
                ...
                self.status_var.set("Iteration #" + str(
                    i) + "; Fraction of thresholded pixels that were analyzed: " + frac_counted_str + " %")
                self.master.update()
            ...
            clusteri_data = {"CLUSTER_ID": i, "SURFACE": surface, "XLIST": xlist.astype(int),
                             "YLIST": ylist.astype(int), "LONG_AXIS": axis, "ROUNDNESS": roundness, "VOLUME": volume,
                             "SHORT_AXIS": short_axis, "XMEAN": xmean, "YMEAN": ymean,
                             "XSTART": X_mask[icurrent].astype(int), "YSTART": Y_mask[icurrent].astype(int),
                             "ZLIST": zlist, "ICLUST_FILTERED": iclust[iclust_filtered], "ICLUST": iclust,
                             "MAXCOST_ALONG_PATH": maxcost_along_path, "COST": cost}
    ...
    def psd_hist_from_data(self, source, hist_color=[147, 36, 30], hist_label=None, bins_input=None, histtype="bar",
                           ypos_errorbar=None, xerr_shift=0.):
        ...
        if bins_input is None:
            ...
            if self.nbins_auto_var.get() == 1:
                ...
                if self.xlog_var.get() == 1:
                    nbins = int(np.ceil(np.log10(float(histrange[1]) - np.log10(histrange[0])) / float(
                        default_log_binsize * default_bin_inflate)))
                else:
                    nbins = int(
                        np.ceil(float(histrange[1] - histrange[0]) / float(default_binsize * default_bin_inflate)))
            else:
                try:
        ...
        ypdf, xpdfleft, patches = plt.hist(data, bins_input, histtype=histtype, color=hist_color_fm, label=hist_label,
                                           weights=data_weights / np.nansum(data_weights), density=False, lw=2,
                                           rwidth=.8)
        ...
        hist_color_fm_error = (
        hist_color[0] / hist_color_fm_error_factor / 255, hist_color[1] / hist_color_fm_error_factor / 255,
        ...
        serrbins = plt.errorbar(xpdf_shifted, ypdf, yerr=np.array([poisson_neg, poisson_pos]), marker=".", markersize=0,
                                linestyle="", color=hist_color_fm_error, elinewidth=2, capsize=0, alpha=0.8, zorder=15)
        ...
        capsize = 3
        capthick = 2
        serr1 = plt.errorbar(avg, ypos_errorbar, xerr=xerr, marker=marker, markersize=markersize * 1.4, linestyle="",
                             color="w", elinewidth=elinewidth + 2, capsize=capsize + 1, capthick=capthick + 1,
                             alpha=0.8, zorder=19)

        serr2 = plt.errorbar(avg, ypos_errorbar, xerr=xerr, marker=marker, markersize=markersize, linestyle="",
                             color=hist_color_fm, elinewidth=elinewidth, ecolor=self.lighter(hist_color_fm, 0.3),
                             markeredgewidth=1.5, markeredgecolor="k", capsize=capsize, capthick=capthick, zorder=20)
    ...
    def create_histogram(self, event):
        ...
        if self.comparison.nclusters is not None:
            self.psd_hist_from_data(self.comparison, hist_color=[74, 124, 179],
                                    hist_label=self.comparison_data_label_var.get(), bins_input=bins_input,
                                    histtype="step", ypos_errorbar=ypos_errorbar * 2, xerr_shift=-xerr_shift)
    ...
    def load_data(self, event):
        ...
        csv_data_filename = filedialog.askopenfilename(initialdir=self.output_dir, title="Select a CSV data file",
    ...
    def save_data(self, event):
        ...
        dataframe = pd.DataFrame({"SURFACE": self.clusters_surface, "ROUNDNESS": self.clusters_roundness,
                                  "SHORT_AXIS": self.clusters_short_axis, "LONG_AXIS": self.clusters_long_axis,
                                  "VOLUME": self.clusters_volume, "PIXEL_SCALE": pixel_scale})
        ...
        if self.expert_mode is True:
            ...
            full_filename = filedialog.asksaveasfilename(initialdir=self.output_dir, initialfile=filename,
                                                         title="Select an output file name")

        # Create a Pandas dataframe for stats
        stats_dataframe = pd.DataFrame(
            {"AVG_DIAM": [float(self.diam_average_var.get())], "STD_DIAM": [float(self.diam_stddev_var.get())],
             "AVG_SURF": [float(self.surf_average_var.get())], "STD_SURF": [float(self.surf_stddev_var.get())],
             "EFF": [float(self.eff_var.get())], "QUAL": [float(self.q_var.get())]})

        # Save files to CSV
        dataframe.to_csv(full_filename)

        # Save stats too
        stats_filename = os.path.dirname(full_filename) + os.sep + os.path.splitext(os.path.basename(full_filename))[
            0] + "_stats.csv"
